...
namespace SF365.Plugins
{
    ...
    {
        public FlightDataVirtualEntityPlugin()
            : base(typeof(FlightDataVirtualEntityPlugin))
        {
        }
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localcontext)
        {
            switch (localcontext.PluginExecutionContext.MessageName)
            {
                case "RetriveMultiple":
                    RetriveMultiple(localcontext);
                    break;
                case "Retrive":
                    Retrive(localcontext);
                    break;
                default:
                    break;
            }
        ...
        private void Retrive(LocalPluginContext localcontext)
        {
            var target = (EntityReference)localcontext
                .PluginExecutionContext
                .InputParameters["Target"];
            var record = GetFlightData(target.Id, null)
                .FirstOrDefault();

            localcontext.PluginExecutionContext
                .OutputParameters["BusinessEntity"] = record;
        }
