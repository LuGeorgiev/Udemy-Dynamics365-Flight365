$env:BUILD_SOURCESDIRECTORY = "C:\Development\Udemy-Dynamics365-Flight365\SECOND-SOLUTION"
...
$files = gci $Env:BUILD_SOURCESDIRECTORY\Solution -recurse -include Solution.xml 
if($files)
{
    Write-Verbose "Will apply $NewVersion to $($files.count) files."
    ...
    foreach ($file in $files) {
        ...
        $filecontent = Get-Content($file)
        attrib $file -r
        $filecontent -replace $VersionRegex, "<Version>$NewVersion</Version>" | Out-File $file  -Encoding UTF8
